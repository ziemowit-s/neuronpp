...
class VecStimCell(CoreCell):
    ...
    def filter_vecstims(self, name: str, obj_filter=None, **kwargs):
        ...
        * Whole object callable function passed to the obj_filter param.
            eg. (lambda expression) returns sections which name contains 'apic' or their distance >
            1000 um from the soma:
          ```
    ...
    def remove_vecstims(self, name: str, obj_filter=None, **kwargs):
        """
        Currently all filter passed are treated as AND statements.
        ...
        * Single object field filter based on callable function passed to the obj_filter param.
        ...
        :return:
        """
        return self.remove(searchable=self.vss, obj_filter=obj_filter, names=name, **kwargs)
    ...
    def filter_vectors(self, name: str, obj_filter=None, **kwargs):
        ...
        * Whole object callable function passed to the obj_filter param.
          ...
          ```
           soma = cell.filter_secs("soma")
           cell.filter_secs(obj_filter=lambda o: 'apic' in o.name or
           h.distance(soma(0.5), o(0.5)) > 1000)
          ```
        ...
        * Single object field filter based on callable function passed to the obj_filter param.
        ...
        :param obj_filter:
            Whole object callable functional filter. If you added also any kwargs they will be
            together with the obj_filter treated as AND statement.
        :return:
        """
        return self.filter(self.vecs, obj_filter=obj_filter, names=name, **kwargs)

    def remove_vectors(self, name: str, obj_filter=None, **kwargs):
        """
        Currently all filter passed are treated as AND statements.
        ...
        * Single object field filter based on callable function passed to the obj_filter param.
        ...
        :return:
        """
        return self.remove(self.vecs, obj_filter=obj_filter, names=name, **kwargs)
    ...
    def make_vecstim(self, ping_array):
        ...
        """
        if h.t > 0:
            raise ConnectionRefusedError("NetStim cannot be created after simulation have been "
                                         "initiated. You need to specify NetStim before creation "
                                         "of SimRun object.")
