...
class PointProcessCell(SectionCell):
    ...
    def remove_point_processes(self, mod_name: str = None, name: str = None, parent: str = None,
                               obj_filter=None, **kwargs):
        """
        Currently all filter passed are treated as AND statements.
        ...
        * Single object field filter based on callable function passed to the obj_filter param.
        ...
        :return:
        """
        return self.remove(searchable=self.pps, obj_filter=obj_filter, mod_name=mod_name, name=name,
                           parent=parent, **kwargs)
    ...
    def _append_pp(self, hoc_point_process, mod_name, segment, tag=None):
        ...
        # take existing parent segment
        parent_segment = segment.parent(hoc_point_process.get_segment().x)
        pp = PointProcess(hoc_point_process, parent=parent_segment, name=result_name, mod_name=mod_name,
                          cell=self)
        self.pps.append(pp)
        return pp
