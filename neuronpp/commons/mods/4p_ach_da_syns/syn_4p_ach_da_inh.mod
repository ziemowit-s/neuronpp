...
ENDCOMMENT
  ...
  NEURON {
  	...
  	:ACh/DA params
  	RANGE ACh_val, Da_val, ACh, ACh_tau, stdp_ach, ach_stdp
  	RANGE Da, Da_tau, stdp_da, da_stdp
  	RANGE ACh_amplitude, Da_amplitude
  ...
  PARAMETER {
    ...
    Da_tau = 50 (ms) <1e-9, 1e9>
  	...
  	ACh_amplitude = 8.5e-7              : amplitude of ACh
  	Da_amplitude = 8.5e-7               : amplitude of Da
  ...
  DERIVATIVE state {
    ...
    ACh_val = ACh * ((last_max_w_Da-Da)/last_max_w_Da) * Eta : Eta only for ACh/Da
    Da_val = Da * Eta
